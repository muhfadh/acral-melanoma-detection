import time
import os
import cnn_svm, cnn_svm_2, cnn_svm_3, cnn_svm_4, cnn_svm_5, cnn_svm_6
import numpy as np
...
# features = []
# names = []
# for feature_path in Path('static/features/Skenario 1 - [90 10]').glob('*.npy'):
#     features.append(np.load(feature_path))
#     img_name = feature_path.stem
#     if 'AM' in img_name:
...
def predict():
    if request.method == 'POST':
        ...
        if chosen_model == 'model1':
            ...
            model4=cnn_svm_4
            labels, runtimes, proba = prediksi(model4, data)
        elif chosen_model == 'model5':
            model5=cnn_svm_5
            labels, runtimes, proba = prediksi(model5, data)
        elif chosen_model == 'model6':
            model6=cnn_svm_6
            labels, runtimes, proba = prediksi(model6, data)
